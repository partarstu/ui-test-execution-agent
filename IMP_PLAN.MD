...
execute strict test steps but make autonomous decisions regarding tools, retries, and error handling.
...
- **Prefetching Strategy**: Implement a "Prefetching" mechanism. While a read-only verification agent is running for Step N, the
  Orchestrator can asynchronously trigger the `ElementLocatorTools` for Step N+1 to "warm up" or find the element, reducing perceived
  latency.
...
- **Self-Correction**: If a tool fails (e.g., "Click" misses), retry with a different strategy (e.g., "Javascript Click") before reporting
  failure.
...
- **Fast-Path Logic**: Agents should prefer faster tools (e.g., Algorithmic Matching) over heavy AI tools (Vision Model) when confidence is
  high.
...
- Monitoring and logging (Agent decisions).
...
- [x] **Implement Location "Prefetching" Logic**
    - Tasks:
        - [x] Verifications are always started in a virtual thread, next test step execution goes further.
        - [x] The agent which executes test step action must have in a prompt an explicit instruction to wait first for any
          verification running, using a corresponding tool, except of cases of locating UI element (i.e. if the test step presumes
          interaction with UI element - agent may invoke the tool to locate the element, but every other tool can't be executed after
          waiting for any running verification is complete). This tool needs to be created in @CommonTools.java, wait for any running
          verification to complete and return a new record instance having boolean field for success of verification and optional String
          message with info from the tool explaining what happened if the verification failed. After that the agent must check if
          the verification result was ok or not. If not ok - the agent must stop test step action execution and return a result of
          test action execution (the corresponding record must be created yet). The Agent.java must then analyze the result of
          test step action and if it's not ok - fail test case execution unless the cause of the failure was exactly unsuccessful
          verification - because such verification will already trigger the test failure anyway.
...
- [x] Configure Retry Policies per agent.

### Phase 10 — Prompt Engineering

- [x] Author and version agent system prompts.
- [x] Implement prompt logging.

### Phase 11 — Configuration and Tuning

- [x] Extend `AgentConfig` (Timeouts, Budgets).
