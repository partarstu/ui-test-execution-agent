...
Phase 5 — Orchestrator and Execution Policies (Sections 1.1, 1.3)
- [x] Extend Agent.java to have missing functionality so that it acts as the Orchestrator:
  - Tasks:
    - [x] Keep context initialization, retry/timeout at orchestration level
    - [x] Iterate preconditions and test steps; delegate to sub-agents for action and verification
    - [x] Maintain screen recording and result aggregation    
      
Phase 6 — Sub-Agents for Action and Verification (Sections 1.2, 6.1)
- [x] Implement sub-agents for: precondition action, precondition verification, test step action, expected results verification
  - Tasks:
    - [x] Provide decision logic for selecting tools and calling UserInteractionService when in attended mode
    - [x] Return ToolExecutionResult<T> with structured payloads
    - [x] Enforce capability scopes (tool allowlists, parameter limits)
  - Acceptance gate:
    - [x] Sub-agents pass unit tests for typical and edge scenarios    

Phase 7 — Agent Communication and Flows (Sections 6.1, 6.2, 6.3)
- [x] Wire orchestrator → sub-agents and sub-agents → UserInteractionService
  - Tasks:
    - [x] Implement flow 6.1 (delegation for preconditions and steps, including verifications)
    - [x] Implement flow 6.2 (agent to UserInteractionService with typed results)
    - [x] Ensure cancellation/interrupt propagation works end-to-end  

Phase 8 — Implement new changes
- [x] Implement
  - Tasks:
...
- [ ] Implement Test Execution Context and bounded element cache
  - Tasks:
    - [ ] Define context object with: test case, execution history of every precondition and test step, current screenshot, timestamps, 
      results history
